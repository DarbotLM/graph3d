"""Tests for graph3d devin install / uninstall commands."""
...
def _skill_path_user(tmp_path):
    return tmp_path / ".config" / "devin" / "skills" / "graph3d" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".devin" / "skills" / "graph3d" / "SKILL.md"


def _rules_path(project_dir):
    return project_dir / ".windsurf" / "rules" / "graph3d.md"


# ---------------------------------------------------------------------------
# User-scope install (graph3d install --platform devin / graph3d devin install)
# ---------------------------------------------------------------------------

def test_devin_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.config/devin/skills/graph3d/SKILL.md."""
...
def test_devin_skill_file_contains_frontmatter(tmp_path):
    ...
    assert "name: graph3d" in content
    assert "argument-hint:" in content
    assert "triggers:" in content


def test_devin_skill_file_references_graph3d_query(tmp_path):
    """/graph3d skill must mention graph3d query (query-first policy)."""
    _devin_install_user(tmp_path)
    content = _skill_path_user(tmp_path).read_text()
    assert "graph3d query" in content or "/graph3d query" in content
...
# ---------------------------------------------------------------------------
# Project-scope install (graph3d devin install --project)
# ---------------------------------------------------------------------------

def test_devin_install_project_creates_skill_file(tmp_path, monkeypatch):
    """Project-scope install copies skill to .devin/skills/graph3d/SKILL.md."""
...
def test_devin_install_project_creates_rules_file(tmp_path, monkeypatch):
    """Project-scope install writes .windsurf/rules/graph3d.md."""
    from graph3d.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    assert rules.exists()
    assert "graph3d" in rules.read_text()
    assert "GRAPH_REPORT.md" in rules.read_text()


def test_devin_rules_content_recommends_graph3d_query(tmp_path):
    """The rules file installed by devin must use query-first policy."""
    from graph3d.__main__ import _devin_rules_install
    _devin_rules_install(tmp_path)
    content = _rules_path(tmp_path).read_text()
    assert "graph3d query" in content
...
def test_devin_uninstall_user_removes_skill_file(tmp_path):
    ...
    from graph3d.__main__ import _remove_skill_file
    with patch("graph3d.__main__.Path.home", return_value=tmp_path):
...
def test_devin_uninstall_user_noop_when_not_installed(tmp_path, capsys):
    ...
    try:
        os.chdir(tmp_path)
        with patch("graph3d.__main__.Path.home", return_value=tmp_path):
            sys.argv = ["graph3d", "devin", "uninstall"]
            main()
    finally:
        os.chdir(old_cwd)
    out = capsys.readouterr().out
...
def test_devin_uninstall_project_removes_skill_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .devin/skills/graph3d/SKILL.md."""
    from graph3d.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_devin_uninstall_project_removes_rules_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .windsurf/rules/graph3d.md."""
    from graph3d.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_devin_rules_uninstall_noop_when_not_installed(tmp_path):
    ...
    from graph3d.__main__ import _devin_rules_uninstall
...
def test_devin_skill_file_exists_in_package():
    ...
    import graph3d
    skill = Path(graph3d.__file__).parent / "skill-devin.md"
...
def test_devin_skill_file_uses_python_c_syntax():
    ...
    All mature graph3d skills use the interpreter-detection pattern
    ``$(cat graph3d-out/.graph3d_python) -c "..."`` rather than bare
    ``python -c "..."`` so they work in pipx / venv environments.
    """
    import graph3d
    skill = (Path(graph3d.__file__).parent / "skill-devin.md").read_text()
    assert '.graph3d_python) -c "' in skill, (
        "skill-devin.md must use the interpreter-detection pattern "
        "'$(cat graph3d-out/.graph3d_python) -c \"...\"'"
    )
...
def test_devin_in_platform_config():
    ...
    from graph3d.__main__ import _PLATFORM_CONFIG
...
def test_devin_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.config/devin/skills/graph3d/SKILL.md."""
    from graph3d.__main__ import _platform_skill_destination
    with patch("graph3d.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("devin", project=False)
    assert dst == tmp_path / ".config" / "devin" / "skills" / "graph3d" / "SKILL.md"


def test_devin_in_main_help_text(capsys, monkeypatch):
    """`graph3d --help` must list devin in the platform list and in the per-platform section."""
    from graph3d.__main__ import main
    ...
    assert "|devin)" in captured or "|devin |" in captured or "|devin" in captured, (
        "devin missing from `graph3d --help` platform list"
    )
...
def test_devin_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.devin/skills/graph3d/SKILL.md."""
    from graph3d.__main__ import _platform_skill_destination
    dst = _platform_skill_destination("devin", project=True, project_dir=tmp_path)
    assert dst == tmp_path / ".devin" / "skills" / "graph3d" / "SKILL.md"
