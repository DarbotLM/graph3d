...
def test_detect_skips_noise_dot_dirs():
    ...
    for files in result["files"].values():
        for f in files:
            # graph3d's own cache is always skipped
            assert "/.graph3d/" not in f
...
def test_graph3dignore_hermetic_without_vcs(tmp_path):
    ...
    # parent .graph3dignore must NOT leak into a non-VCS scan
...
def test_graph3dignore_at_git_root_is_included(tmp_path):
    ...
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".graph3dignore").write_text("vendor/\n")
...
def test_detect_incremental_propagates_follow_symlinks(tmp_path, monkeypatch):
    ...
    # Store manifest inside graph3d-out/ so it is pruned by _SKIP_DIRS
    # and doesn't get re-detected as a code file now that .json is indexed.
    manifest_dir = tmp_path / "graph3d-out"
    manifest_dir.mkdir()
...
def test_detect_converts_google_workspace_shortcuts_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("graph3d.detect.convert_google_workspace_file", fake_convert)
...
def test_detect_skips_graph3d_own_cache(tmp_path):
    ...
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert not any(".graph3d" in f for f in all_files)
    assert any("app.py" in f for f in all_files)
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    (tmp_path / ".graph3dignore").write_text("android/\n!src/\n")
...
def test_negation_works_when_no_ancestor_excluded(tmp_path):
    ...
    keep = src / "keep.py"
    keep.write_text("x = 1")
    (tmp_path / ".graph3dignore").write_text("*.py\n!src/keep.py\n")
    patterns = _load_graph3dignore(tmp_path)
...
def test_anchored_file_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".graph3dignore").write_text("/build\n")
    patterns = _load_graph3dignore(tmp_path)
...
def test_anchored_multi_segment_pattern(tmp_path):
    ...
    (tmp_path / ".graph3dignore").write_text("/src/inbox/\n")
    patterns = _load_graph3dignore(tmp_path)
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from graph3d.cache import save_cached
...
def test_graph3dignore_takes_precedence_over_gitignore(tmp_path):
    ...
    (tmp_path / ".graph3dignore").write_text("other.py\n")
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("other.py" in f for f in code)  # graph3dignore IS applied
