...
```
/graph3d                     # full pipeline on current directory
/graph3d <path>              # full pipeline on specific path
/graph3d <path> --update     # incremental - re-extract only new/changed files
/graph3d <path> --no-viz     # skip visualization, just report + JSON
/graph3d <path> --wiki       # build agent-crawlable wiki
/graph3d query "<question>"  # BFS traversal - broad context
```

## What You Must Do When Invoked

If the user invoked `/graph3d --help` or `/graph3d -h` (with no other arguments), print the contents of the `## Usage` section above verbatim and stop. Do not run any commands, do not detect files, do not default the path to `.`. Just print the Usage block and return.
...
### Step 1 - Ensure graph3d is installed

```python
python -c "import graph3d; import sys; from pathlib import Path; Path('graph3d-out').mkdir(exist_ok=True); Path('graph3d-out/.graph3d_python').write_text(sys.executable)"
```

If the import fails, install first:

```python
python -m pip install graph3d -q
```
...
Path('graph3d-out/.graph3d_detect.json').write_text(json.dumps(result, indent=2))
...
detect = json.loads(Path('graph3d-out/.graph3d_detect.json').read_text())
code_files = []
...
if code_files:
    ...
    Path('graph3d-out/.graph3d_ast.json').write_text(json.dumps(result, indent=2))
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('graph3d-out/.graph3d_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}))
...
if cached_nodes or cached_edges:
    Path('graph3d-out/.graph3d_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}))
Path('graph3d-out/.graph3d_uncached.txt').write_text('\n'.join(uncached))
...
```
You are a graph3d extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
ast = json.loads(Path('graph3d-out/.graph3d_ast.json').read_text())
all_nodes.extend(ast.get('nodes', []))
all_edges.extend(ast.get('edges', []))

cached_path = Path('graph3d-out/.graph3d_cached.json')
...
Path('graph3d-out/.graph3d_extract.json').write_text(json.dumps(merged, indent=2))
...
extraction = json.loads(Path('graph3d-out/.graph3d_extract.json').read_text())
...
Path('graph3d-out/graph.json').write_text(json.dumps(graph_data, indent=2))
Path('graph3d-out/.graph3d_analysis.json').write_text(json.dumps({
...
extraction = json.loads(Path('graph3d-out/.graph3d_extract.json').read_text())
analysis = json.loads(Path('graph3d-out/.graph3d_analysis.json').read_text())
...
Path('graph3d-out/GRAPH_REPORT.md').write_text(report)
...
from graph3d.build import build_from_json
from graph3d.cluster import cluster
from graph3d.export import to_html
from pathlib import Path
...
try:
    to_html(G, communities, 'graph3d-out/graph.html')
...
Read `graph3d-out/GRAPH_REPORT.md` and share the **God Nodes** and **Surprising Connections** sections directly in the chat — do not ask the user to open the file themselves.
