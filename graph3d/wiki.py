...
def _community_article(
    ...
    lines += ["---", "", "*Part of the graph3d knowledge wiki. See [[index]] to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by graph3d. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [graph3d](https://github.com/DarbotLM/graph3d)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `graph3d extract .` or `graph3d cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `graph3d extract .` to regenerate .graph3d_analysis.json."
        )
