...
Markdown sidecars under `graph3d-out/converted/`. Office files (`.docx`,
`.xlsx`) use the `[office]` extra. Google Workspace shortcuts (`.gdoc`,
`.gsheet`, `.gslides`) are opt-in with `--google-workspace` or
`GRAPH3D_GOOGLE_WORKSPACE=1` and require an authenticated `gws` CLI.
...
| graph3d source + Transformer paper | 4 | **5.4x** |
...
Every extracted file is fingerprinted by content hash. Re-runs skip unchanged files entirely — only new or modified files go through extraction again. The cache lives in `graph3d-out/cache/`.
