...
**Une compétence pour assistant de code IA.** Tapez `/graph3d` dans Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro ou Google Antigravity — il lit vos fichiers, construit un graphe de connaissances et vous révèle une structure que vous ne voyiez pas auparavant. Comprenez une base de code plus rapidement. Trouvez le « pourquoi » derrière les décisions architecturales.

Entièrement multimodal. Déposez du code, des PDFs, du markdown, des captures d'écran, des diagrammes, des photos de tableau blanc, des images dans d'autres langues, ou des fichiers vidéo et audio — graph3d extrait les concepts et les relations de tout cela et les connecte en un seul graphe. Les vidéos sont transcrites localement avec Whisper grâce à un prompt adapté au domaine. 30+ langages de programmation supportés via tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy maintient un dossier `/raw` où il dépose des articles, tweets, captures d'écran et notes. graph3d est la réponse à ce problème — 71,5 fois moins de tokens par requête versus la lecture des fichiers bruts, persistant entre les sessions, honnête sur ce qui a été trouvé versus déduit.

```
/graph3d .                        # fonctionne sur n'importe quel dossier — code, notes, articles, tout
```
...
Ajoutez un fichier `.graph3dignore` pour exclure des dossiers :
...
> **Package officiel :** Le package PyPI s'appelle `graph3d` (installer avec `pip install graph3d`). Les autres packages nommés `graph3d*` sur PyPI ne sont pas affiliés à ce projet. Le seul dépôt officiel est [DarbotLM/graph3d](https://github.com/DarbotLM/graph3d).
...
| Claude Code (Windows) | `graph3d install` (détection automatique) ou `graph3d install --platform windows` |
...
Note : Codex utilise `$` au lieu de `/` pour les compétences, tapez donc `$graph3d .`.
...
```
/graph3d                          # répertoire courant
/graph3d ./raw                    # dossier spécifique
/graph3d ./raw --mode deep        # extraction d'arêtes INFERRED plus agressive
/graph3d ./raw --update           # ne réextraire que les fichiers modifiés
/graph3d ./raw --directed         # graphe dirigé
/graph3d ./raw --cluster-only     # relancer le clustering sur le graphe existant
/graph3d ./raw --no-viz           # pas d'HTML, juste rapport + JSON
/graph3d ./raw --obsidian         # générer un vault Obsidian (opt-in)

/graph3d add https://arxiv.org/abs/1706.03762   # récupérer un article
/graph3d add <video-url>                         # télécharger l'audio, transcrire, ajouter
/graph3d query "qu'est-ce qui connecte Attention à l'optimiseur ?"
/graph3d path "DigestAuth" "Response"
/graph3d explain "SwinTransformer"

graph3d hook install              # installer les hooks Git
graph3d update ./src              # réextraire les fichiers de code, sans LLM
graph3d watch ./src               # mise à jour automatique du graphe
```
...
**Hooks Git** (`graph3d hook install`) — installe des hooks post-commit et post-checkout.

## Confidentialité

graph3d envoie le contenu des fichiers à l'API du modèle de votre assistant IA pour l'extraction sémantique des docs, articles et images. Les fichiers de code sont traités localement via tree-sitter AST. Les fichiers vidéo et audio sont transcrits localement avec faster-whisper. Aucune télémétrie, aucun suivi d'utilisation.
...
## Construit sur graph3d — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) est la couche enterprise au-dessus de graph3d. Là où graph3d transforme un dossier de fichiers en graphe de connaissances, Penpax applique le même graphe à toute votre vie professionnelle — en continu.
