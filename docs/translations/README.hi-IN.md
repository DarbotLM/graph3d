...
**एक AI कोडिंग असिस्टेंट स्किल।** Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro या Google Antigravity में `/graph3d` टाइप करें — यह आपकी फ़ाइलें पढ़ता है, एक नॉलेज ग्राफ बनाता है, और आपको वह संरचना वापस देता है जो आप नहीं जानते थे कि मौजूद है। कोडबेस को तेज़ी से समझें। आर्किटेक्चरल निर्णयों के पीछे का "क्यों" खोजें।

पूरी तरह मल्टीमोडल। कोड, PDFs, मार्कडाउन, स्क्रीनशॉट, डायग्राम, व्हाइटबोर्ड फोटो, अन्य भाषाओं में छवियां, या वीडियो और ऑडियो फ़ाइलें डालें — graph3d इन सभी से अवधारणाएं और संबंध निकालता है और उन्हें एक ग्राफ में जोड़ता है। वीडियो को Whisper से स्थानीय रूप से ट्रांसक्राइब किया जाता है। 25 प्रोग्रामिंग भाषाएं tree-sitter AST के माध्यम से समर्थित हैं (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart)।

> Andrej Karpathy एक `/raw` फोल्डर रखते हैं जहां वह papers, tweets, स्क्रीनशॉट और नोट्स डालते हैं। graph3d उस समस्या का जवाब है — रॉ फ़ाइलें पढ़ने की तुलना में प्रति क्वेरी **71.5x** कम tokens, सत्रों में स्थायी, ईमानदार कि क्या पाया गया बनाम अनुमान लगाया गया।

```
/graph3d .                        # किसी भी फोल्डर पर काम करता है — कोडबेस, नोट्स, papers, सब कुछ
```
...
अनचाहे फोल्डर को बाहर करने के लिए `.graph3dignore` फ़ाइल जोड़ें:
...
> **आधिकारिक पैकेज:** PyPI पैकेज का नाम `graph3d` है (`pip install graph3d` से इंस्टॉल करें)। PyPI पर `graph3d*` नाम वाले अन्य पैकेज इस प्रोजेक्ट से संबद्ध नहीं हैं। एकमात्र आधिकारिक रिपॉजिटरी [DarbotLM/graph3d](https://github.com/DarbotLM/graph3d) है।
...
| Claude Code (Windows) | `graph3d install` (स्वतः-पहचान) या `graph3d install --platform windows` |
...
```
/graph3d                          # वर्तमान डायरेक्टरी
/graph3d ./raw                    # विशिष्ट फोल्डर
/graph3d ./raw --update           # केवल बदली हुई फ़ाइलें फिर से निकालें
/graph3d ./raw --directed         # निर्देशित ग्राफ
/graph3d ./raw --no-viz           # केवल रिपोर्ट + JSON
/graph3d ./raw --obsidian         # Obsidian vault बनाएं

/graph3d add https://arxiv.org/abs/1706.03762   # paper प्राप्त करें
/graph3d add <video-url>                         # वीडियो ट्रांसक्राइब करें
/graph3d query "attention और optimizer को क्या जोड़ता है?"
/graph3d path "DigestAuth" "Response"
/graph3d explain "SwinTransformer"

graph3d hook install              # Git hooks इंस्टॉल करें
graph3d update ./src              # कोड फ़ाइलें पुनः निकालें, LLM की जरूरत नहीं
graph3d watch ./src               # स्वचालित ग्राफ अपडेट
```
