...
**Una habilidad para asistentes de código IA.** Escribe `/graph3d` en Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro o Google Antigravity — lee tus archivos, construye un grafo de conocimiento y te devuelve estructura que no sabías que existía. Entiende una base de código más rápido. Encuentra el «por qué» detrás de las decisiones arquitectónicas.

Totalmente multimodal. Deposita código, PDFs, markdown, capturas de pantalla, diagramas, fotos de pizarras, imágenes en otros idiomas, o archivos de video y audio — graph3d extrae conceptos y relaciones de todo ello y los conecta en un solo grafo. Los videos se transcriben localmente con Whisper usando un prompt adaptado al dominio derivado de tu corpus. 30+ lenguajes de programación soportados mediante tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy mantiene una carpeta `/raw` donde deposita papers, tweets, capturas de pantalla y notas. graph3d es la respuesta a ese problema — 71,5 veces menos tokens por consulta versus leer los archivos sin procesar, persistente entre sesiones, honesto sobre lo que encontró versus lo que infirió.

```
/graph3d .                        # funciona con cualquier carpeta — tu código, notas, papers, todo
```
...
Añade un archivo `.graph3dignore` para excluir carpetas:
...
> **Paquete oficial:** El paquete PyPI se llama `graph3d` (instalar con `pip install graph3d`). Otros paquetes llamados `graph3d*` en PyPI no están afiliados con este proyecto. El único repositorio oficial es [DarbotLM/graph3d](https://github.com/DarbotLM/graph3d).
...
| Claude Code (Windows) | `graph3d install` (detección automática) o `graph3d install --platform windows` |
...
Nota: Codex usa `$` en lugar de `/` para habilidades, así que escribe `$graph3d .`.
...
```
/graph3d                          # directorio actual
/graph3d ./raw                    # carpeta específica
/graph3d ./raw --mode deep        # extracción de aristas INFERRED más agresiva
/graph3d ./raw --update           # re-extraer solo archivos modificados
/graph3d ./raw --directed         # grafo dirigido
/graph3d ./raw --cluster-only     # re-ejecutar clustering en grafo existente
/graph3d ./raw --no-viz           # sin HTML, solo informe + JSON
/graph3d ./raw --obsidian         # generar vault de Obsidian (opt-in)

/graph3d add https://arxiv.org/abs/1706.03762   # obtener un paper
/graph3d add <video-url>                         # descargar audio, transcribir, añadir
/graph3d query "¿qué conecta Attention con el optimizador?"
/graph3d path "DigestAuth" "Response"
/graph3d explain "SwinTransformer"

graph3d hook install              # instalar hooks de Git
graph3d update ./src              # re-extraer archivos de código, sin LLM
graph3d watch ./src               # actualización automática del grafo
```
...
**Hooks de Git** (`graph3d hook install`) — instala hooks post-commit y post-checkout.

## Privacidad

graph3d envía contenido de archivos a la API del modelo de tu asistente IA para extracción semántica de documentos, papers e imágenes. Los archivos de código se procesan localmente mediante tree-sitter AST. Los archivos de video y audio se transcriben localmente con faster-whisper. Sin telemetría, sin seguimiento de uso.
...
## Construido sobre graph3d — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) es la capa enterprise sobre graph3d. Donde graph3d convierte una carpeta de archivos en un grafo de conocimiento, Penpax aplica el mismo grafo a toda tu vida laboral — continuamente.
