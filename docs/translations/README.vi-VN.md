...
**Kỹ năng dành cho trợ lý lập trình AI.** Gõ `/graph3d` trong Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro hoặc Google Antigravity — nó đọc các tệp của bạn, xây dựng đồ thị kiến thức và trả lại cho bạn cấu trúc mà bạn không biết là tồn tại. Hiểu codebase nhanh hơn. Tìm ra "tại sao" đằng sau các quyết định kiến trúc.

Hoàn toàn đa phương thức. Thêm code, PDF, markdown, ảnh chụp màn hình, sơ đồ, ảnh bảng trắng, hình ảnh bằng ngôn ngữ khác hoặc tệp video và âm thanh — graph3d trích xuất các khái niệm và mối quan hệ từ tất cả mọi thứ và kết nối chúng trong một đồ thị duy nhất. Video được phiên âm cục bộ bằng Whisper. Hỗ trợ 30+ ngôn ngữ lập trình qua tree-sitter AST.

> Andrej Karpathy duy trì một thư mục `/raw` nơi anh ấy đặt các bài báo, tweet, ảnh chụp màn hình và ghi chú. graph3d là câu trả lời cho vấn đề đó — **71,5x** ít token hơn trên mỗi truy vấn so với đọc các tệp thô, liên tục giữa các phiên.
...
> **Gói chính thức:** Gói PyPI có tên là `graph3d`. Kho lưu trữ chính thức duy nhất là [DarbotLM/graph3d](https://github.com/DarbotLM/graph3d).
...
/graph3d .
/graph3d ./raw --update
/graph3d query "điều gì kết nối Attention với optimizer?"
...
## Được xây dựng trên graph3d — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) là lớp doanh nghiệp trên graph3d. **Dùng thử miễn phí sắp ra mắt.** [Tham gia danh sách chờ →](https://safishamsi.github.io/penpax.ai)
