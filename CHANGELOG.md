...
- Feat: native PDF engine (`graph3d/pdf.py`) replaces the `pypdf` dependency entirely — a from-scratch reader implementing classic xref tables, cross-reference streams and object streams (PDF 1.5+), a brute-force `N G obj` recovery scan for damaged files, FlateDecode/ASCIIHex/ASCII85/RunLength filters, page-tree traversal, simple-font encoding resolution (StandardEncoding/WinAnsiEncoding/MacRomanEncoding plus `/Differences`), and ToUnicode CMap parsing for Type0/CID fonts. Validated against a real-world 25-page datasheet PDF (word-count parity with pypdf: ratio 1.02) plus hand-built fixtures covering every cross-reference format. Also ships a document-intelligence layer (`get_summary`, `analyze_structure`, `detect_document_type`, `search_text`, `extract_tables`, `to_markdown`) ported from the Darbot PDF Viewer MCP extension (github.com/darbotlabs/Darbot-PDF-Viewer-MCP), improved to use real per-page text instead of that tool's form-feed-splitting heuristic. `pypdf` removed from the `pdf` extra and from the dependency tree.
...
- Feat: custom LLM provider registry — `graph3d provider add/list/show/remove` registers any OpenAI-compatible endpoint (NVIDIA NIM, vLLM, OpenRouter, Together, LiteLLM) via `~/.graph3d/providers.json`; custom providers auto-detected after built-ins in `detect_backend()` priority (#1084)
...
- Fix: `pnpm-workspace.yaml` with `packages: - '.'` no longer crashes with `IndexError: tuple index out of range` on Python 3.10 — `Path.glob('.')` replaced with `[root]` guard in `_load_workspace_packages`; `GRAPH3D_DEBUG=1` env var added to `_safe_extract` for full traceback on extraction errors (#1083)
- Fix: anchored `.graph3dignore` patterns (leading `/`) no longer match the same directory name anywhere in the tree — `_matches()` in both `_is_ignored` and `_is_included` now gates basename/segment shortcuts on `not anchored`; anchored patterns do exact anchor-relative path match only (#1087)
...
- Fix: `graph3d install antigravity` global install now writes to `~/.gemini/config/skills/` (per Antigravity docs) instead of the wrong `~/.agents/`; uninstall, version-stamp refresh, and project-scope install all updated to match (#1079)
...
- Fix: `graph3d-out/.graph3d_python` path prefix was missing in 8 skill files (256 instances) causing `cat: .graph3d_python: No such file or directory` on every non-Claude-Code platform
- Chore: all skill files now use uv-aware interpreter detection — `uv tool run graph3d python` preferred over shebang parsing when uv is available
...
- Feat: `graph3d extract --mode deep` flag enables richer semantic extraction using an extended system prompt; flag propagated through all four LLM backends (#1030)

## 0.8.21 (2026-05-27)

- Fix: `graph3d update` (no `--changed` flag) no longer leaves ghost nodes from files deleted between runs — full re-extraction path now reconciles the existing graph against current disk state and evicts any node whose `source_file` no longer exists; `_norm_source_file` used on both sides to guarantee path format consistency (#1007)
- Fix: `graph3d install --platform opencode --project` now writes `SKILL.md` to `.opencode/skills/graph3d/SKILL.md` (discoverable by OpenCode) instead of the incorrect `.config/opencode/skills/` path; git-add hint updated accordingly (#1040)
- Fix: post-commit hook no longer triggers rebuild when only `graph3d-out/` files were committed (avoids infinite dirty-tree loop when graph outputs are tracked in git); `GRAPH3D_SKIP_HOOK=1` env var added for one-off skip; hook rebuild log now appends (`>>`) instead of overwriting (`>`) (#1018, #1037)
...
- Feat: Amp (ampcode.com) platform support — `graph3d amp install/uninstall` installs the skill into `.amp/skills/graph3d/SKILL.md` (#948)
...
- Fix: `graph3d-out/memory/` files are no longer silently excluded by `.gitignore` pattern matching — memory dir files now bypass the gitignore filter in `detect.py`, ensuring knowledge accumulated via `graph3d remember` is always scanned (#1047)

## 0.8.20 (2026-05-26)

- Fix: stale nodes persist after `graph3d update` when files are deleted on Windows — `deleted_paths` and `evict_sources` in `_rebuild_code` now use `.as_posix()` for consistent forward-slash paths; `_relativize_source_files` called on the existing graph before eviction (not after); `_relativize_source_files` itself now produces forward slashes (#1007)
- Fix: `graph3d extract` stale-node pruning now also handles symlinked scan roots — `prune_set` expansion uses `Path(root).resolve()` before `relative_to()` so symlinked roots produce correct relative paths (#1007)
...
- Fix: `cluster-only` no longer drops community label alignment after re-clustering — `remap_communities_to_previous` now applied in the `cluster-only` path, matching the behaviour of `graph3d update` (#1028)
...
- Feat: Chinese query segmentation — compound Chinese tokens (e.g. `页面路由`) are split into meaningful words using jieba when installed, with character bigram fallback; original compound preserved alongside segments for exact-match; new `pip install "graph3d[chinese]"` extra (#1026)
...
- Feat: Devin CLI support — `graph3d devin install/uninstall` installs the skill into Devin's `.devin/rules/` directory (#1020)
...
- Fix: `graph3d export` (html/obsidian/wiki/svg/graphml/neo4j) no longer collapses to "Single community" when `.graph3d_analysis.json` is absent — falls back to per-node `community` attribute already present in `graph.json` (#1001)
...
- Fix: `graph3d` now exits non-zero when all semantic-extraction chunks fail — previously a silent empty graph was written with exit code 0, masking backend failures (#889)
- Feat: `graph3d install --project` installs the skill into the current repository (`.claude/skills/`, `.agents/skills/`, etc.) instead of the user home directory; per-platform subcommands support the same flag (#931)
- Docs: Uzbek (uz-UZ) README translation (#982)

## 0.8.15 (2026-05-22)

- Fix: `cluster-only` subcommand crashed with `FileNotFoundError` when `graph3d-out/` did not yet exist — output directory is now created before any write (#934)
- Fix: `GRAPH3D_MAX_OUTPUT_TOKENS` env var now respected for all OpenAI-compatible backends — previously the token limit was hardcoded, causing truncated responses on high-context queries (#973)
...
- Fix: `.gitignore` patterns now respected when no `.graph3dignore` exists — previous behaviour silently ignored the project's gitignore, causing expected exclusions to be skipped (#945)
- Feat: `--exclude <pattern>` CLI flag to pass extra gitignore-style exclusion patterns at runtime without modifying `.graph3dignore` (#947)
...
- Feat: `graph3d cache-check`, `graph3d merge-chunks`, `graph3d merge-semantic` CLI subcommands expose cache and merge logic as library-callable commands for skill pipelines
...
- Fix: `--watch` now respects `.graph3dignore` — the event handler was checking extensions before the ignore filter, so paths inside `node_modules/`, `.venv/`, etc. triggered rebuilds (#928)
- Fix: `graph3d <path>` now correctly dispatches to `graph3d extract <path>` — previously a bare path argument returned "unknown command" instead of starting extraction
- Fix: skill fast path — if `graph3d-out/graph.json` already exists and the request is a natural-language question, extraction steps are skipped entirely and `graph3d query` runs immediately; previously the skill re-ran detect and hit the corpus-size gate on every question
...
- Fix: Codex skill now uses graph3d query/explain/path even when graph artifacts are dirty in worktree (#913, closes #860)
...
- Fix: install guidance now directs users to run `/graph3d query` interactively rather than reading `GRAPH_REPORT.md` first; the report is a summary, not a starting point (#891)

## 0.8.5 (2026-05-15)

- Fix: `.graph3dignore` parent-exclusion rule now correctly blocks files under an excluded directory even when a `!` negation exists elsewhere in the file — previously any negation pattern disabled directory pruning entirely (#882)
...
- Fix: Windows skill temp files (chunk JSONs, `.graph3d_python`, `.graph3d_root`) no longer pollute the project root — all written under `graph3d-out/` (#831)
- Fix: `--update` with deletions-only no longer errors when `.graph3d_extract.json` does not yet exist — creates an empty extraction file before merging (#876)

## 0.8.2 (2026-05-15)

- Fix: Python interpreter detection for `uv tool` and `pipx` installs on Windows — `graph3d install` and all skill steps now find the correct executable (#831)
- Fix: antigravity Windows skill path resolution (#831)
- Fix: dot directories (e.g. `.github/`, `.vscode/`) are now indexed when explicitly included via `.graph3dignore` (#873)
...
- Feat: Mermaid callflow HTML auto-regenerates on every graph rebuild when `*-callflow.html` exists in `graph3d-out/` — works with `--watch` and `graph3d hook install`
...
- Fix: `graph3d hook install` now works in git linked worktrees — uses `git rev-parse --git-path hooks` instead of constructing `.git/hooks/` directly (#865)
- Fix: office sidecar files in `graph3d-out/converted/` are now checked against `.graph3dignore` before being added to the file list (#861)
- Fix: `save_manifest()` accepts a `kind` parameter (`ast`, `semantic`, `both`) — incremental AST-only `graph3d update` no longer overwrites `semantic_hash` entries, preventing spurious full re-extracts on the next run (#857)
- Fix: five paths in `skill-windows.md` Step B3 were missing the `graph3d-out/` prefix, causing chunk files to be written to the wrong directory (#862)
...
- Docs: skill.md now clarifies that graph3d does not read `ANTHROPIC_API_KEY` or other provider keys during `/graph3d` skill runs — the host IDE session provides the LLM (PR #864)

## 0.7.18 (2026-05-14)

- Fix: `graph3d update` is now idempotent — graph.json and GRAPH_REPORT.md are only rewritten when content actually changes; topology comparison short-circuits clustering entirely on unchanged graphs, eliminating residual community-count drift (#824)
- Fix: community IDs are now stable across rebuilds — Leiden/Louvain receive deterministically sorted input and a fixed random seed; greedy overlap remapper preserves existing IDs so hand-edited `.graph3d_labels.json` labels don't drift onto wrong communities (#824)
- Fix: `--no-cluster` flag added to `graph3d update` — writes raw AST graph without clustering, consistent with `graph3d extract --no-cluster` (#824)
- Fix: `graph3d update --no-cluster` now writes `"links"` key matching the schema of the full clustered path; previously wrote `"edges"`, causing schema toggle on every mode switch
- Fix: `.graph3d_labels.json` was rewritten on every rebuild even when nothing changed; now only written when outputs actually change
...
- Fix: node ID format in skill.md corrected to `{parent_dir}_{filename_stem}_{entity}` — the old filename-only format caused ghost-duplicate nodes when AST and semantic extractors disagreed on the stem; top-level files use just the filename stem; existing graphs with ghost duplicates can be cleaned up with `graph3d extract --force`
...
- Fix: `graph3d path` and `graph3d explain` now render arrow direction correctly — `-->` for caller→callee, `<--` for callee←caller; previously the graph was loaded undirected so every hop printed `-->` regardless of stored direction (#849, #853)
...
- Fix: `graph3d extract --backend bedrock` was rejected by the CLI guard even when `AWS_PROFILE`/`AWS_REGION`/`AWS_DEFAULT_REGION`/`AWS_ACCESS_KEY_ID` were set — boto3 session auth was never reached (#846)
...
- Fix: Step 1 install fallback in skill now prefers `uv tool install --upgrade graph3d` over `pip` when uv is on PATH — pip was installing to the wrong environment when graph3d was originally installed via `uv tool` (#831)
- Fix: `_score_nodes` in `serve.py` now uses three-tier precedence (exact 1000 / prefix 100 / substring 1) instead of flat substring scoring — `graph3d path "Foo" "FooBar"` no longer returns 0 hops when both labels substring-match the same node (#828)
- Fix: `graph3d path` and MCP `_tool_shortest_path` now emit a clear error when source and target resolve to the same node, instead of silently returning 0 hops (#828)
...
- Fix: `_AGENTS_MD_SECTION` now includes the `/graph3d` skill trigger instruction — all 7 AGENTS.md platforms (OpenCode, Codex, Aider, Trae, Hermes, OpenClaw, Factory Droid) now correctly invoke the skill tool when the user types `/graph3d` (#827)

## 0.7.15 (2026-05-11)

- Fix: `-h`/`--help`/`-?` in any position now stops execution — previously `graph3d cursor install --help` silently installed into Cursor; `graph3d benchmark --help` crashed with FileNotFoundError (#821)
- Fix: `--version`, `-v`, and `graph3d version` now print the installed version and exit (#818)
- Fix: `GRAPH3D_OLLAMA_NUM_CTX=<invalid>` no longer falls back to hardcoded 131072 (which exhausted VRAM) — it now falls through to the auto-derived value and prints a warning (#820)
- Fix: when `GRAPH3D_OLLAMA_NUM_CTX` is set smaller than the estimated chunk size, graph3d now warns explicitly that Ollama will silently truncate the prompt and suggests a corrected `--token-budget` (#820)
...
- Fix: subagent chunk files are now written to an absolute path (`CHUNK_PATH` injected at dispatch time from `graph3d-out/.graph3d_root`) so the Write tool doesn't lose chunks to an undefined working directory (#808)
...
- Fix: hollow-response warning now mentions VRAM pressure and `GRAPH3D_OLLAMA_NUM_CTX` / `GRAPH3D_OLLAMA_KEEP_ALIVE` env vars as tuning knobs (#798)
- Feat: `graph3d export callflow-html` -- generates a self-contained Mermaid architecture/call-flow HTML page from `graph3d-out/graph.json`, grouped by community with interactive zoom/pan diagrams, call detail tables, and graph report highlights (#797)
...
- Fix: `graph3d explain` and `graph3d path` no longer crash on `MultiGraph` inputs -- new `edge_data()`/`edge_datas()` helpers in `build.py` handle both simple and multi-graphs; all 8 production call sites and 30 skill-file inline heredocs updated (#796)
...
- Fix: post-commit hook no longer spawns unbounded parallel rebuilds -- per-repo `fcntl.flock` non-blocking lock in `_rebuild_code`; `changed_paths` wired from hook through to AST extractor; stale nodes evicted on deletion; `GRAPH3D_REBUILD_TIMEOUT` watchdog; Darwin-aware memory cap (#791)
...
- Feat: `graph3d extract` gains `--max-workers`, `--token-budget`, `--max-concurrency`, `--api-timeout` flags; hard 8-worker AST cap removed; explicit HTTP timeout on OpenAI client (default 600s, `GRAPH3D_API_TIMEOUT`); ollama API key gate skipped for loopback URLs (#792)
...
- Feat: `/graph3d --help` now prints the Usage block and stops without running pipeline steps (all 12 skill files) (#795)
...
- Feat: `graph3d uninstall` top-level command -- removes graph3d skill files from all platforms in one shot; `--purge` flag also deletes `graph3d-out/`
...
- Feat: AWS Bedrock backend -- `graph3d extract ./docs --backend bedrock`; credentials via standard AWS provider chain (AWS_PROFILE, AWS_REGION, IAM roles, SSO); model via GRAPH3D_BEDROCK_MODEL (default anthropic.claude-3-5-sonnet-20241022-v2:0); `[bedrock]` extra adds boto3 (#757)
...
- Feat: Gemini and OpenAI backends -- `graph3d extract ./docs --backend gemini` (GEMINI_API_KEY / GOOGLE_API_KEY) or `--backend openai` (OPENAI_API_KEY); `[gemini]` and `[openai]` extras added (#735)
...
- Fix: `cluster-only` now loads and saves `.graph3d_labels.json` -- human-readable community labels survive re-clustering instead of resetting to "Community N" (#744)
- Fix: `graph3d export wiki` now fails fast with exit 1 if `.graph3d_analysis.json` is missing -- prevents silent deletion of existing wiki articles (#746)
...
- Feat: Ollama backend for headless extraction -- `graph3d extract ./docs --backend ollama`; auto-detected when `OLLAMA_BASE_URL` is set; defaults to `qwen2.5-coder:7b`; zero cost ($0.00); sentinel API key handles OpenAI client auth requirement (#729)
- Feat: Cross-project global graph at `~/.graph3d/global.json` -- `graph3d global add/remove/list/path` to register multiple project graphs with `<repo>::<id>` prefixed node IDs, preventing silent collisions; hash-based skip avoids re-ingesting unchanged graphs (#729)
- Feat: `graph3d extract --global --as <tag>` flag -- after building a project graph, auto-registers it into the global graph in one step (#729)
...
- Fix: `graph3d extract --backend claude/kimi` raises default `max_tokens` from 8192 → 16384, eliminating the truncation-then-recursive-split cascade on dense doc corpora; respects `GRAPH3D_MAX_OUTPUT_TOKENS` env var (#730)
...
- Fix: `graph3d extract` (full rebuild path) now saves `manifest.json` on every successful run, not only on `--update`; prevents stale-manifest drift on subsequent incremental runs (#538)
- Fix: `graph3d antigravity install` now writes to `.agent/` (no trailing s) matching Antigravity's actual config paths (#704)
...
- Feat: `graph3d extract` now runs incrementally - auto-detects prior `manifest.json` and re-extracts only changed/new files; semantic results cached by content hash so unchanged docs cost zero LLM tokens on repeat runs (#698)
...
- Feat: `--dedup-llm` flag for `graph3d extract` - optional LLM tiebreaker for ambiguous entity pairs (~$0.01 for 10k-node graphs), off by default
- Fix: `graph3d hook install` rebuild now preserves human-readable community labels from `.graph3d_labels.json` instead of resetting to generic "Community N" names on every commit (#705)
- Fix: `graph3d install --platform gemini` now works correctly (#706)
...
- Feat: `graph3d hook install` now also configures a git merge driver for `graph3d-out/graph.json` — union-merges two graph.json files so git never produces conflict markers in the knowledge graph; writes `.gitattributes` and registers `graph3d merge-driver` in `.git/config`
- Feat: `graph3d merge-driver <base> <current> <other>` subcommand — takes two graph.json variants and writes their node/edge union back to `<current>`; always exits 0 so merge never blocks
...
- Feat: `GRAPH3D_OUT` env var overrides the output directory — accepts a relative name or absolute path, wires through `cache.py`, `watch.py`, and the CLI; useful for sharing one graph across multiple git worktrees (#686)
- Fix: `graph3d antigravity install` now auto-updates stale rules and workflow files on re-run instead of silently skipping them (#652)
- Docs: README simplified — less dense, plain language; technical pipeline details moved to `docs/how-it-works.md`

## 0.6.8 (2026-05-03)

- Fix: `.graph3dignore` negation patterns (`!src/**`) now work correctly — when any `!` pattern is present, directory pruning is deferred to per-file checks so negated files inside ignored directories are reached (#676)
- Fix: Antigravity slash command `/graph3d` now appears in the command dropdown — workflow file now includes YAML frontmatter with `name: graph3d` required for Antigravity discovery (#678)
...
- Fix: `graph3d install --platform codex` now writes absolute path to `graph3d` executable — fixes PATH resolution in VS Code extension on Windows (#651)
...
- Feat: `graph3d tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe (#557)
...
- Feat: `.graph3dinclude` hidden path allowlist — opt specific hidden dirs into traversal (e.g. `.hermes/plans/**/*.md`) (#583)
- Feat: `--no-viz` flag wired in `cluster-only`; `GRAPH3D_VIZ_NODE_LIMIT` env var overrides 5000-node HTML threshold (#565)
...
- Feat: `graph3d tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe via `html.escape()` and `_js_safe()` (#557)
...
- Feat: `graph3d update --force` and `GRAPH3D_FORCE=1` env var — bypass the node-count safety check after refactors that legitimately shrink the graph (#639)
- Fix: Codex PreToolUse hook on Windows — replaced `python3 -c "..."` inline command (fails on Conda where only `python` exists, and breaks PowerShell JSON parsing) with `graph3d hook-check`, a new shell-agnostic subcommand. Re-run `graph3d codex install` to regenerate the hook (#651, #522)
...
- Fix: incremental rebuild (`graph3d update`, post-commit hook) dropped INFERRED/AMBIGUOUS semantic nodes extracted from code files — node preservation now filters by ID membership in the new AST output instead of `file_type`, so LLM-extracted call/data-flow edges survive code-only rebuilds (#653)
- Fix: post-commit and post-checkout hooks blocked `git commit` for the full rebuild duration (hours on large repos) — rebuilds now detach via `nohup & disown`, git returns in ~100ms, log written to `~/.cache/graph3d-rebuild.log` (#650)
...
- Fix: `graph3d update` / `graph3d watch` never persisted the manifest, so every subsequent `--update` re-extracted all files — manifest now saved after each rebuild (#621)
- Fix: inline comments in `.graph3dignore` (e.g. `vendor/ # legacy`) now stripped correctly — whitespace + `#` suffix is treated as a comment, `path#hash.py` preserved (#605)
- Fix: `graph3d query "FunctionName"` now returns the exact matching node first instead of high-degree hub modules hijacking the output — 100-point exact-match bonus + seeds render before BFS expansion (#638)
...
- Fix: `.graph3dignore` negation patterns (`!src/lib/secrets.ts`) now work — full last-match-wins evaluation with `!` un-ignore support (#628)

## 0.6.1 (2026-05-01)

- Fix: `.graph3dignore` discovery now uses correct gitignore semantics — outer rules are loaded first so inner (closer) rules always win via last-match-wins, matching standard gitignore behavior (#643)
- Fix: without a VCS root, `.graph3dignore` discovery is now hermetic to the scan folder — no leakage across sibling projects in a shared workspace (#643)
- Fix: anchored patterns (leading `/`) in a parent `.graph3dignore` now correctly apply only relative to their own directory, not the scan root (#643)
...
- Feat: SQL AST extractor — `.sql` files now processed deterministically via tree-sitter. Extracts tables, views, functions/procedures, foreign key references, and FROM/JOIN reads_from edges. No LLM needed. Requires `pip install 'graph3d[sql]'` (#349)
...
- Fix: `NameError: name '_os' is not defined` crash after `graph3d update` — this was fixed in v5 branch but not released to PyPI (#618, #612)

## 0.5.5 (2026-04-29)

- Feat: Kimi K2.6 backend — `pip install 'graph3d[kimi]'` + `MOONSHOT_API_KEY` routes semantic extraction through Kimi K2.6. 3-6x richer relation extraction at ~3x lower cost. Claude remains default; Kimi is opt-in.
...
- Fix: `graph3d update` remembers scan root via `graph3d-out/.graph3d_root` — no path argument needed on subsequent runs
...
- Fix: community labels deleted in Step 9 cleanup — `.graph3d_labels.json` is now preserved so wiki/obsidian/HTML retain human-readable names after re-cluster (#608)
- Fix: `NameError: name '_os' is not defined` in `graph3d update` Kimi tip (#612)
...
- Feat: `graph3d clone <github-url>` — clone and graph any public repo
- Feat: `graph3d merge-graphs` — combine multiple `graph.json` outputs into one cross-repo graph
- Feat: `CLAUDE_CONFIG_DIR` support in `graph3d install`
...
- Fix: `graph3d-out/` excluded from source scanning

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `graph3d install` when multiple platforms were previously installed — `graph3d install` now refreshes `.graph3d_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/graph3d-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `graph3d cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/graph3d --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.graph3d_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `graph3d update .` and `graph3d cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `graph3d-out/graph3d-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when graph3d is run from a subdirectory (#410)
- Fix: `graph3d hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `graph3d-out/`, `.graph3dignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: graph3d watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `graph3d vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `graph3d watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `graph3d-out/wiki/` (#229, #354)
- Fix: `graph3d install --platform opencode` now also installs the `.opencode/plugins/graph3d.js` plugin, matching what `graph3d opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/graph3d-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `graph3d update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `graph3d update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `graph3d kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `graph3d update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the graph3d binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `graph3d query` and `graph3d explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `graph3d kiro install` writes `.kiro/skills/graph3d/SKILL.md` (invoked via `/graph3d`) and `.kiro/steering/graph3d.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `graph3d query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `graph3d query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `graph3d-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.graph3dignore` patterns from a parent directory now fire correctly when graph3d is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.graph3dignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `graph3d install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `graph3d hermes install` writes skill to `~/.hermes/skills/graph3d/SKILL.md` and AGENTS.md (#251)
...
- Fix: `graph3d claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `graph3d`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `graph3d antigravity install` writes `.agent/rules/graph3d.md` (always-on rules) and `.agent/workflows/graph3d.md` (`/graph3d` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has graph3d installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.graph3dignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graph3d transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/graph3d add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `graph3d-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'graph3d[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graph3d transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/graph3d add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `graph3d-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'graph3d[video]'` for faster-whisper + yt-dlp
...
- Fix: graph3d install --platform gemini now also copies the skill file to ~/.gemini/skills/graph3d/SKILL.md so the /graph3d trigger works in Gemini CLI (#174)
...
- Fix: `graph3d install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `graph3d install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_graph3dignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `graph3d install --platform aider` copies skill to `~/.aider/graph3d/SKILL.md`; `graph3d aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `graph3d install --platform copilot` copies skill to `~/.copilot/skills/graph3d/SKILL.md`; `graph3d copilot install/uninstall` for skill management (#134)
...
- Fix: `graph3d codex install` (and opencode) no longer exits early when `AGENTS.md` already has the graph3d section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `graph3d gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `graph3d cursor install` writes `.cursor/rules/graph3d.mdc` with `alwaysApply: true` so the graph context is always included; `graph3d cursor uninstall` removes it (#137)
...
- Fix: README clarifies `graph3d` is the only official PyPI package — other `graph3d*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `graph3d opencode install` now writes `.opencode/plugins/graph3d.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.graph3dignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `graph3d save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `graph3d save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p graph3d-out` now runs before writing `.graph3d_python` in `skill.md` — prevents write failure on first run; `.graph3d_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `graph3d query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.graph3d_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`graph3d install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from graph3d binary)
...
- Fix: `graph3d codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.graph3d_*.json`) now live in `graph3d-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `graph3d install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$graph3d .` not `/graph3d .` (#36)
...
- Add: `graph3d query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `graph3d-out/obsidian`
...
- Add: `graph3d install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `graph3d codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `graph3d claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `graph3d claude install` — writes graph3d section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `graph3d claude uninstall` — removes section and hook
- Add: `graph3d hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `graph3d hook uninstall` / `hook status`
- Add: `graph3d benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (graph3d-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if graph3d-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what graph3d can do
...
- Add: `graph3d/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install graph3d` → `pip install graph3d` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → graph3d answer framing
...
- Memory feedback loop - Q&A results saved to `graph3d-out/memory/`, extracted on `--update`
...
- `graph3d install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
